...
- サイト用リポジトリのGitHub Actionsで、content sources取得後に`tools/check-language-pairs`で日英ページ対応検査を実行し、必要な検査・生成、Antora buildを実行する。
...
- `repository_dispatch`で起動した公開workflowは、content sources取得後に`tools/check-language-pairs`で日英ページ対応検査を実行し、必要な検査・生成、Antora build、GitHub Pages deployを実行する。
